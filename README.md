...
a standalone playable web build. Four complete games — `games/collect-room`,
`games/physics-targets`, `games/gem-rush` (built by a separate agent from the documented
contract alone), and `games/omagatoki-circuit` (a three-lap anti-gravity combat racer on the
`racer.*` behavior family, regenerated by `pnpm racer`) — run on that shared runtime with
registered behaviors, input, a HUD, and game rules.

...
a standalone playable web build. Two showcase games run on that shared runtime with registered
behaviors, input, a HUD, and game rules:

| Game | Explore |
| --- | --- |
| [NEON YOMI](games/neon-yomi/README.md) | Anti-gravity combat racing over a drowned city: a 5.6 km circuit, two laps, three selectable hulls, three rivals, boost pads and weapons. |
| [Physics Targets](games/physics-targets/README.md) | A small shooting gallery: launch a ball, knock over five targets, and tune the physics in the inspector. |

Open either project from the editor's project list. Collect Room and Gem Rush remain under
`tests/fixtures/projects/` for regression checks; starter templates remain in `templates/`.
...
- `pnpm verify:stage2` — 19/19 checks (assets, clips, property controls, keyboard focus, drag cancellation)
- `pnpm verify:stage3` — 14/14 checks (Collect Room and Physics Targets played end to end, wall contact, tuning editable)
- `pnpm verify:stage4` — 17/17 checks (agent-built game, human edit, independence, management)
- `pnpm verify:stage5` — 24/24 checks (compatibility, resource ownership, reference-scene
...
NEON YOMI running as a standalone exported game. Open its source in the editor to explore the
track, imported models, materials, and gameplay settings.

![NEON YOMI gameplay: racing, boost pads, the city circuit, and the finish](docs/showcase/neon-yomi.png)
...
- **http://127.0.0.1:5178/player.html?project=./games/neon-yomi/** — NEON YOMI as a plain player
  page, loading its project from files exactly as an export does. Swap in `physics-targets`
  for the smaller physics demo; the default workspace is `games/`.

`Ctrl+C` stops both processes. The two games in `games/` are the worked examples; `templates/`
holds the starter projects `pnpm studio create` copies from.

### Running an exported game

```bash
pnpm studio build neon-yomi     # writes games/neon-yomi/.coilbox/export/
npx --yes serve games/neon-yomi/.coilbox/export
```
...
pnpm verify:stage3    # stage 3 gate: plays Collect Room and Physics Targets in a headless browser
...
pnpm racer            # regenerate the retired circuit in .scratch/legacy-games/
...
tests/        unit tests, the stage 0 check suite, and fixtures (including retired demo projects)

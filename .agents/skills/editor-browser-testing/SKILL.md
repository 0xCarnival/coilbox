...
## History and command-state testing

- Scope Create to the top toolbar or ToolDock: both controls have the same accessible name. History also has distinct tab and rail-button roles.
- Enlarge the bottom panel through the separator named `Resize the bottom panel` so current and undone rows remain visible together.
- History rows represent cumulative state after the named edit. Test origin restoration, a forward jump that visibly restores an entity, and a new branch that removes the old future rows.
- For one-notification checks, passively subscribe to the document store and record history positions around real row clicks. Do not invoke session commands or mutate the document to produce test states.
- Compare saved/reloaded documents with a captured baseline, accounting for service-owned revision increments; history itself restarts when a scene is opened.
